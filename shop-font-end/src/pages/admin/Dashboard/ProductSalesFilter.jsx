...
const ProductSalesFilter = () => {
  ...
  // Hàm gọi API
  const fetchProductSalesData = async (type = "month", startDate, endDate) => {
    try {
      ...
      const response = await instance.get("/stats/getProductSalesByTime", {
        params,
        headers: {
  ...
  const handleApplyFilter = async () => {
    if (filterType === "custom" && customDateRange.length === 2) {
      ...
      fetchProductSalesData(filterType);
    }
  };
  ...
  return (
    <div>
      <div className="flex gap-2 mb-3">
        <p className="font-bold text-[20px]">Sản phẩm bán được</p>
        <Space>
          <Select
            defaultValue="month"
      ...
      <p>
        <strong>Tổng số sản phẩm bán được:</strong>{" "}
        {productSalesData?.totalProductsSold}
      </p>
    </div>
  );
};

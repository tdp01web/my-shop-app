...
const HeaderLayoutAdmin = () => {
  ...
  const handleSearch = async () => {
    ...
    if (query.trim().length > 0) {
      ...
      if (results) {
        navigate("/staff/products", { state: { query, results } });
      } else {
        setErrorMessage("Không tìm thấy sản phẩm nào.");
        navigate("/staff/NotSearch", { state: { query } });
      }
    }
  };
  ...
  const menu = (
    <Menu style={{ width: 200}}>
      ...
      <Menu.Item key="account" onClick={() => navigate(`/staff/users/${user._id}/edit`)}>
  ...
  return (
    ...
    >
      <div className="flex flex-1 justify-between items-center">
        <Link to="/staff/sales" className="flex items-center w-[190px]">
        ...
        <div>
          <Dropdown overlay={menu} trigger={['hover']}>
            ...
            >
              {`Nhân viên ${user.lastName} ${user.firstName}`}
            </Typography.Title>
          </Dropdown>
        </div>
      </div>
